...
export function requireAdminConsoleAuth(req: { headers?: any }) {
  ...
  const secret = process.env.ADMIN_CONSOLE_AUTH_SECRET;
  if (!secret) return { ok: false as const, status: 500, error: "Missing ADMIN_CONSOLE_AUTH_SECRET" };
